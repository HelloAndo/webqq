...
function toDial(obj){
	...
	var iDeg=30;
	for(var i=0;i<12;i++){
		sHtml+="<span style='-webkit-transform:rotate("+iDeg*i+"deg) translate("+55*Math.sin(2*Math.PI/360)+"px, "+55*Math.cos(2*Math.PI/360)+"px) '></span>" ;
	}
	obj.innerHTML=sHtml;
}
...
oWeatherMenuPro.onmousedown=oWeatherMenuCity.onmousedown=function(e){
	var e=e||event;
	e.stopPropagation();
	e.cancelBubble=true;
};
// oWeatherMenuPro.onmouseover=function(e){
// 	oWeather.onmousedown=null;
// }
// oWeatherMenuPro.onmouseout=function(e){
// 	drap(oWeather);
// }
...
oWeatherMenuConfirm.onclick=function(){
	...
	oCityName.innerHTML=oWeatherMenuCity.value;
	getWeather(citymap[oWeatherMenuPro.value][oWeatherMenuCity.value]);
		// drap(oWeather);
...
// console.log(_this.flag)
					// startMove(that, {left: l, top: t}, 20);
